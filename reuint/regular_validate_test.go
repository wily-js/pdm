...
func TestSnValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{"CASE1", "123", false},
		{"CASE2", "", false},
		{"CASE3", "123123123123123123", true},
		{"CASE4", "XXXXXXXXXXXXXXXXXX", false},
		{"CASE5", "123123qweqwe1312312q", false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := SnValidate(tt.str); got != tt.want {
				t.Errorf("SnValidate() = %v, want %v", got, tt.want)
			}
		})
	}
}
