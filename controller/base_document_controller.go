...
// NewBaseDocumentAreaController 创建基础文档区
func NewBaseDocumentAreaController(router gin.IRouter) *BaseDocumentAreaController {
	res := &BaseDocumentAreaController{}
	r := router.Group("/baseDocArea")
	// 列表
...
// BaseDocumentAreaController 基础文档区控制器
type BaseDocumentAreaController struct {
}

/**
@api {GET} /api/baseDocArea/attr 查看属性
@apiDescription 查看文件或文件夹的属性。
@apiName BaseDocAreaAttr
@apiGroup BaseDocArea
...
GET /api/baseDocArea/attr?path=/%E6%A0%87%E5%87%86/
...
// attr 查看属性
func (c *BaseDocumentAreaController) attr(ctx *gin.Context) {
	fileDir, _ := url.QueryUnescape(ctx.Query("path"))

	p := filepath.Join(dir.BaseDocAreaDir, fileDir)
	if !strings.HasPrefix(p, dir.BaseDocAreaDir) {
...
/**
@api {POST} /api/baseDocArea/copy 复制
@apiDescription 复制文件/文件夹。
@apiName BaseDocAreaCopy
@apiGroup BaseDocArea
...
// copy 复制
func (c *BaseDocumentAreaController) copy(ctx *gin.Context) {
	...
	from := filepath.Join(dir.BaseDocAreaDir, fileCopy.From)
	if !strings.HasPrefix(from, dir.BaseDocAreaDir) || from == dir.BaseDocAreaDir {
		ErrIllegal(ctx, "文件路径错误")
		return
	}
	info, _ := os.Stat(from)
	to := filepath.Join(dir.BaseDocAreaDir, fileCopy.To)
	if !strings.HasPrefix(to, dir.BaseDocAreaDir) || to == dir.BaseDocAreaDir || (info.IsDir() && strings.HasPrefix(to, from+string(filepath.Separator))) {
		ErrIllegal(ctx, "文件路径错误")
		return
	}
	_, err = os.Stat(to)
...
/**
@api {DELETE} /api/baseDocArea/remove 删除
@apiDescription 删除文件或文件夹。
@apiName BaseDocAreaRemove
@apiGroup BaseDocArea
...
DELETE /api/baseDocArea/remove?path=/%E6%A0%87%E5%87%86/
...
// remove 删除
func (c *BaseDocumentAreaController) remove(ctx *gin.Context) {
	fileDir, _ := url.QueryUnescape(ctx.Query("path"))
	...
	p := filepath.Join(dir.BaseDocAreaDir, fileDir)
	if !strings.HasPrefix(p, dir.BaseDocAreaDir) || p == dir.BaseDocAreaDir {
		ErrIllegal(ctx, "文件路径错误")
		return
	}
...
/**
@api {POST} /api/baseDocArea/move 移动
@apiDescription 移动或重命名文件/文件夹。
@apiName BaseDocAreaMove
@apiGroup BaseDocArea
...
// move 移动或重命名文件
func (c *BaseDocumentAreaController) move(ctx *gin.Context) {
	...
	param.From = filepath.Join(dir.BaseDocAreaDir, param.From)
	param.To = filepath.Join(dir.BaseDocAreaDir, param.To)
	if !strings.HasPrefix(param.From, dir.BaseDocAreaDir) || param.From == dir.BaseDocAreaDir {
	...
	if !strings.HasPrefix(param.To, dir.BaseDocAreaDir) || param.To == dir.BaseDocAreaDir || (info.IsDir() && strings.HasPrefix(param.To, param.From+string(filepath.Separator))) {
...
/**
@api {GET} /api/baseDocArea/list 列表
@apiDescription 返回指定目录下所有的文件或目录。
@apiName BaseDocAreaList
@apiGroup BaseDocArea
...
@apiParamExample {get} 请求示例
GET /api/baseDocArea/list?base=/%E6%A0%87%E5%87%86/
...
// list 查看文件列表
func (c *BaseDocumentAreaController) list(ctx *gin.Context) {
	...
	p := filepath.Join(dir.BaseDocAreaDir, baseDir)
	if !strings.HasPrefix(p, dir.BaseDocAreaDir) {
	...
	_ = filepath.Walk(p, func(path string, info fs.FileInfo, err error) error {
		...
		item.Path = strings.Replace(path, dir.BaseDocAreaDir, "", 1)
...
/**
@api {GET} /api/baseDocArea/download 下载
@apiDescription 下载文件或文件夹（文件夹以打包成压缩包的形式下载）。
@apiName BaseDocAreaDownload
@apiGroup BaseDocArea
...
path= area/1,2,3,4
@apiParamExample {get} 请求示例
GET /api/baseDocArea/download?path=/%E6%A0%87%E5%87%86/,/%E6%8A%A5%E5%91%8A/
...
*/
// download 下载文件
func (c *BaseDocumentAreaController) download(ctx *gin.Context) {
	...
	for i, filename := range fileList {
		filePath := filepath.Join(dir.BaseDocAreaDir, filename)
		if !strings.HasPrefix(filePath, dir.BaseDocAreaDir) || filePath == dir.BaseDocAreaDir {
			ErrIllegal(ctx, "路径错误")
			return
		}
		fileList[i] = filePath
...
/**
@api {POST} /api/baseDocArea/upload 上传
@apiDescription 以表单的方式上传文件列表。

@apiName BaseDocAreaUpload
@apiGroup BaseDocArea
...
// upload 上传文件
func (c *BaseDocumentAreaController) upload(ctx *gin.Context) {
	// 获取表单文件
	...
	for _, file := range files {
		_ = os.MkdirAll(filepath.Join(dir.BaseDocAreaDir, base), os.ModePerm)
		filePath := filepath.Join(dir.BaseDocAreaDir, base, file.Filename)
...
/**
@api {POST} /api/baseDocArea/mkdir 创建目录
@apiDescription 创建目录，若包含多个级则递归创建。
@apiName BaseDocAreaMkdir
@apiGroup BaseDocArea
...
*/
// 创建文件夹
func (c *BaseDocumentAreaController) mkdir(ctx *gin.Context) {
	var base string
	...
	p := filepath.Join(dir.BaseDocAreaDir, base)
	if !strings.HasPrefix(p, dir.BaseDocAreaDir) || p == dir.BaseDocAreaDir {
		ErrIllegal(ctx, "路径错误")
		return
	}
...
/**
@api {GET} /api/baseDocArea/search 搜索
@apiDescription 返回所有指定目录（递归子文件）下包含关键字名称的文件或目录。
@apiName BaseDocAreaSearch
@apiGroup BaseDocArea
...
@apiParamExample {get} 请求示例
GET /api/baseDocArea/search?base=/%E6%A0%87%E5%87%86/&keyword=%E6%8A%A5%E5%91%8A
...
// search 搜索文件
func (c *BaseDocumentAreaController) search(ctx *gin.Context) {
	baseDir, _ := url.QueryUnescape(ctx.Query("base"))
	keyword, _ := url.QueryUnescape(ctx.Query("keyword"))
